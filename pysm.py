...
def scale_freqs(c, o, pol=None, samples=10.):
    ...
    if c.spectral_model == "cmb":
        if o.bandpass == False:
            return convert_units(['u', 'K_CMB'], o.output_units, o.output_frequency)[np.newaxis, :, np.newaxis]
        else:
            return (1. / freq_cen ** 2)[..., np.newaxis] * np.sum((freq ** 2)[..., np.newaxis] * convert_units(['u', 'K_CMB'], o.output_units, freq)[..., np.newaxis], axis=np.ndim(freq) - 1) / samples
