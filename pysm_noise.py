...
def instrument_noise(fname_config):
    ...
    # converts size of pixel from steradians to square arcminutes
    ...
    instrument_noise = np.random.randn(3, np.asarray(out.output_frequency).size,
                                       npix)

    #standard_normal*sigma+mu = N(mu, sigma)
    instrument_noise[0, ...] = sigma_pix_i[np.newaxis, :, np.newaxis] * \
                               instrument_noise[0, ...]

    instrument_noise[1, ...] = sigma_pix_pol[np.newaxis, :, np.newaxis] * \
                               instrument_noise[1, ...]
