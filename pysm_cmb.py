""" This contains code is edited from taylens code: Naess, S. K. and Louis, T. 2013
...
def main(fname_config):
    ...
    if CMB.compute_lensed_cmb:
        ...
        # Reading input spectra in CAMB format.
        # CAMB outputs l(l+1)/2pi hence the corrections.

        cl_tebp_arr = np.zeros([10, lmax_cl + 1])
        # 0=TT, 1=EE, 2=BB, 4=TE, 5=EB, 7=TB
        cl_tebp_arr[0, 2:] = 2 * np.pi * data[1]/(l[2:] * (l[2:] + 1))
        cl_tebp_arr[1, 2:] = 2 * np.pi * data[2]/(l[2:] * (l[2:] + 1))
        cl_tebp_arr[2, 2:] = 2 * np.pi * data[3]/(l[2:] * (l[2:] + 1))
        cl_tebp_arr[4, 2:] = 2 * np.pi * data[4]/(l[2:] * (l[2:] + 1))
        cl_tebp_arr[5, :] = np.zeros(lmax_cl + 1)
        cl_tebp_arr[7, :] = np.zeros(lmax_cl + 1)

        if CMB.delens:
            # 3=PP, 6=BP, 8=EP, 9=TP
            cl_tebp_arr[3, 2:] = 2 * np.pi * data[5] * CMB.delensing_ells[1]/(l[2:] * (l[2:] + 1)) ** 2
            cl_tebp_arr[6, :] = np.zeros(lmax_cl + 1)
            cl_tebp_arr[8, 2:] = 2 * np.pi * data[7] * np.sqrt(CMB.delensing_ells[1])/(l[2:] * (l[2:] + 1)) ** 1.5
            cl_tebp_arr[9, 2:] = 2 * np.pi * data[6] * np.sqrt(CMB.delensing_ells[1])/(l[2:] * (l[2:] + 1)) ** 1.5
        else:
            # 3=PP, 6=BP, 8=EP, 9=TP
            cl_tebp_arr[3, 2:] = 2 * np.pi * data[5]/(l[2:] * (l[2:] + 1)) ** 2
            cl_tebp_arr[6, :] = np.zeros(lmax_cl + 1)
            cl_tebp_arr[8, 2:] = 2 * np.pi * data[7]/(l[2:] * (l[2:] + 1)) ** 1.5
            cl_tebp_arr[9, 2:] = 2 * np.pi * data[6]/(l[2:] * (l[2:] + 1)) ** 1.5
