...
def main(fname_config):
    ...
    cfg_fname += config.get('SpinningDust', 'model') + '_config.ini'
    ...
    scaled_map_spdust = sm.scale_freqs(spdust1, out, pol=False) * \
                        spdust1.em_template * unit_conversion1 + \
                        sm.scale_freqs(spdust2, out, pol=False) * \
                        spdust2.em_template * unit_conversion2

    scaled_map_spdust_pol = scaled_map_spdust[np.newaxis, ...] * \
                            np.asarray([np.cos(pol_angle), np.sin(pol_angle)])[:, np.newaxis, :] * spdust_general.pol_frac

    if out.debug is True:
        for i in range(0, len(out.output_frequency)):
            out_fname = out.output_dir
            out_fname += 'spdust_%d.fits' % (out.output_frequency[i])

            hp.write_map(out_fname, scaled_map_spdust[i],
                         coord='G', column_units=out.output_units)

    return np.concatenate([scaled_map_spdust[np.newaxis, ...],
                           scaled_map_spdust_pol])
